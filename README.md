...
- **Administration** (subnav under **Platform → Administration** for **ADMIN** users): API tokens, **Sign-in & identity** (local email/password + optional single IdP: LDAP, Microsoft Entra, OIDC), user management, audit, docs, health. Identity is configured in the UI and/or `AUTH_*` environment variables (see [`docs/design-auth-user-management.md`](docs/design-auth-user-management.md) and [`platform/.env.example`](platform/.env.example)); run Prisma migrations so `Organization.identityConfig` exists.
